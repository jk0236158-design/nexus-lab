...
- キリル文字の混入 (= публish / работ / протокол 型、 Fable 5 のクセ、 6/11〜7/11 で 4 回実測 = 3 回昇格条件の履行)。 日英混在の長文で技術語 1 語が丸ごと露文字化する。 露文字に触れる話題では発生率が上がる (4 回目は混入の記録文の中で発生)。 書いた直後に目視、 zen_stop_hook 検出器が二重に拾う、 見つけたら即修正 + zen_self_improvement_2026-06-11.md 実例 3 に追記
