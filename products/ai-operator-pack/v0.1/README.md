...
> ⚠️ 開発中です。 まだ販売開始していません。 観察負荷試験 (= 第 1 段階、 2026-05-08〜2026-05-21 = 14 日完了) は経過しました。 ただし公開判断ゲート (= 第 6 段階、 「公開する / しないを二択で決定」 = 動いた記録に基づく) は **未通過**、 結論は jun + Kai + Zen の三者判断待ち状態です (= 2026-05-27 時点)。
...
- **道具自身が見つける改善** (自己改善): Yuino 自身が 「ここをこうした方がいい」 を見つけて表に出す仕組み
...
| 第 2 段階: 自走の繰り返しの完成 (期日なし) | Kai 依頼 → Yuino 判断 → Zen 返信 → 結果記録 の一周 | 完了条件: 二重起動の抑止 + 結果点検の動作 |
...
- Yuino 側: 次の作業を見つける・整理・仕分け・送る・記録する役割
- AI 側 (Claude / Codex 等): 受け取った作業を実行・点検・修復・結果を返す役割
- 夜の時間帯も、 「危なくない内部作業」 (= 緑) は止まらず続く。 「外に出す」 「お金が動く」 「公開する」 系 (= 赤) は jun の確認待ちで止まります。
...
全員 Claude Opus 4.7 / Sonnet 4.6 上で動く AI です。 Aira / Yuino を 「自分たちで使う」 形 (= 自社で使う検証) で、 道具自身が良くなる構造を観察しながら作っています。
