...
> ⚠️ v0.1 では起動コマンド (`npm run yuino:start`) はまだ実装途中です (公開判断ゲート (第 6 段階) 通過時に動くようになる予定)。 この項目は公開判断通過後に確認できるようになります。
...
Discord でリアルタイムに質問できる場所を準備中です。 公開判断ゲート (第 6 段階) 通過後に開設予定です。 今は GitHub issue でお願いします。
...
- Yuino の demo の使い方: `docs/execution/yuino_demo.md` (観察試験の第 1 段階 期間中に Kai 起稿予定)
- 仕組みの説明: `docs/execution/architecture.md` (観察試験の第 1 段階 期間中に Kai 起稿予定)
