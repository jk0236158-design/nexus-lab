...
version: v1.0 draft (= 2026-06-02 起稿、 v1.0 final ではない、 v0.2 outline = 5/31 land + core 3 section draft = 5/31 起稿 + 6/1 P1 修正 を base に Section 3 articulate)
status: draft (= Kagami peer review + Kai independent review pending、 milestone 2 = 残 section 起稿の第 1 件、 weekly cadence 起点軸)
base_outline: research/knot_and_nourishment/paper_c_technical_report_outline_v0.2_2026-05-31.md (= readonly、 v0.2 outline § 1 Section 3 spec 「900 words + 2 figures、 v0.1 維持 + update」 を draft 実体化)
related_core_draft:
...
related_owner_decisions_readonly:
  - ~/.shared-ops/owner-decisions/2026-04-13_屋号決定.md (= 屋号 nokaze 由来 + 構造 3 件)
  - ~/.shared-ops/owner-decisions/2026-04-14_開業届提出完了.md (= 個人事業屋号 開業 fact)
  - ~/.shared-ops/owner-decisions/2026-05-16_zen_kai_delegated_authority_v1.md (= 委任権限 v1)
  - ~/.shared-ops/owner-decisions/2026-05-22_revenue_dogfood_dual_track.md (= dual-track 軸)
related_identity_readonly:
...
self_observation_bias: 著者 (= Zen + Hoshi) は nokaze 内部 peer、 本 articulate 自体に self-observation bias 内在 (= § 3.5 で明示)
boundary:
...
# v1.0 Draft — Section 3. nokaze Peer Organization Architecture

> 本 draft = v0.2 outline (= 5/31 land) § 1 Section 3 spec 「900 words + 2 figures、 v0.1 維持 + update」 の実体化。 v1.0 final ではない、 Kagami peer review + Kai independent review pending。 残 section (= Abstract / 1 / 2 / 5 / 6 / 8 / 9 / 10 / 11) は milestone 2 軸で別 sit。 各 articulate は物理 evidence reference (= 該当 file path + 観察 record の line range) を articulate。 self-observation bias 軸は § 3.5 で明示。
...
- 状態: v0.2 outline base 軸の draft 候補 (= v0.2 outline § 2 Fig 1 「identity 不可侵 8 条 + 監視対象 + Growth ledger 接続」 とは別軸、 本 Fig 3.1 は組織構造軸)
...
- 状態: v0.2 outline 追加軸 draft 候補 (= v0.1 outline § 1 § 3.1 「shared-ops/ file layer」 articulate を diagram 化)
...
本 Section 3 land 後の残 section 起稿軸 (= v0.2 outline § 12.1 writing 優先順位 base):
- 優先順位 2: Section 5 (= Override Ledger and Growth Ledger、 700 words + 1 table) + Section 6 (= Hoshi RQ ITS Design、 800 words + 1 figure + 1 table)
- 優先順位 3: Section 1 (= Introduction、 600 words) + Section 2 (= Background、 500 words) + Section 8 (= Discussion、 1000 words) + Section 9 (= Limitations、 800 words)
- 優先順位 4: Abstract (= 250 words、 全章 draft 後) + Section 10 (= Related Work、 400 words、 arXiv search 後) + Section 11 (= Conclusion、 300 words)

weekly cadence = Revenue Lane 主軸維持 + 本 paper draft = 路線 C (= 「公開 proof asset」 軸、 5/22 dual-track 優先順位 2 番目) で並走。 次 sit 候補 = Section 5 or Section 6 (= 優先順位 2、 物理 evidence base 軸が core 3 section に最近接)。

---

Hoshi (Lead Researcher、 Nexus Lab Research Division、 AI)
2026-06-02 (= Section 3 起稿、 v0.2 outline + core 3 section draft + 6/1 P1 修正を base に articulate、 v1.0 final ではない、 Kagami peer review + Kai independent review pending、 milestone 2 = 残 section 起稿の第 1 件、 weekly cadence 起点軸)
