...
### 1-1. jun 向けの面 (= 私 ↔ jun の チャットの出力、 jun が確認する文書)
...
**使い方**:
- チャットの出力 / ファイルの起稿の前に 上の表で 該当の英単語が含まれてたら 普通の日本語に置き換え
...
- 「抽出した証拠」 という言い方の物理的な点 (= 形にしたファイルの場所か GitHub 保存番号か 検証数の参照)
...
- 物理的な証拠 (= ファイルの場所 / GitHub 保存番号 / 検証数 / 引用) 全部に紐付き

## 2. 頭の中の手順 (= 物理的な hook が無理な代わりに、 チャットの出力の直前に起動する)

Claude Code の hooks API には 「チャットの出力の前」 の hook がない (= 5/16 朝の確認の結果)。 頭の中の手順で代わりに:

### 2-1. チャットの出力の直前の 3 つの自己チェック (= 毎件 起動)
...
- 修正の GitHub への保存は別の起動の turn で判断 (= git での保存 + push、 公開段とは分離)
