...
## 1. 対外公開の 200 確認の習慣 (2026-04-19 起票、 宣言と実装のズレ再発防止) `[頭の中]`
...
## 2. Zenn の 404 と週次上限の見分け (2026-04-23 起票、 Kagami が指摘した阻害要因 3) `[頭の中]`
...
2. **4 本以上** = Zenn の週次上限が確定。 空の保存を push しても受け付けてくれないので意味がない
   ...
   - `~/.shared-ops/inbox/<date>_zen_zenn_rate_limit_retry_<retry_date>.md` を起票 (既定: 7 日間の枠が空く日、 期限: 既定+1 日)
...
## 5. 第 5 の問い + 5 軸の見直し点検 (5/09 PM 起票、 黙って待つ失敗が 2 連発火した後) `[頭の中]`
...
## 6. ファイル字数の上限 = 3000 字以下 (4.7 既定の 「文章ダンプを避ける」 + 4 ヶ月初心者の読み手に整合) `[頭の中]`
...
- `docs/rules/communication.md` (チャット出力系の頭の中の決まりの集まり、 4.5 + 4.6 と整合)
...
- `~/.claude/projects/c--Users-jk023-nexus-lab/memory/feedback_honesty_violation_exaggeration.md` (誇張禁止の軸)
- `~/.claude/projects/c--Users-jk023-nexus-lab/memory/feedback_publish_before_dogfood_premature.md` (公開前の自社使用に整合)
