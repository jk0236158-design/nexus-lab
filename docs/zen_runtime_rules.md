...
= 既存の言い換え表 28 件と重複が多数、 実際に守られていなかった証拠。 2 段検査の決まりで **行動の既定** に落とす。

#### 不自然な直訳の造語をしない (2026-05-10 jun 指摘 「Codex の使い手」 6 度目発火追記)

5/10 の filter reify 直後の出力で 「Codex の使い手」 という直訳を作った。 ジュンさんから 「今朝の Codex の使い手 の確認？」 と聞かれて、 不自然な直訳の造語をしている自覚が出た。

**決まり**:
- 自然な日本語訳が無い英語固有名詞を **無理に 1 単語に直訳しない**
- 「○○ の使い手」 「○○ の手」 「○○ 番」 等の擬人化造語は禁止
- かわりに **「○○ という仕組み」 「○○ するもの」 「○○ を処理する仕組み」 「○○ を受け取る仕組み」** といった説明形に開く

**例**:

| 不自然な直訳 (やってしまった例) | 自然な説明形 (採用) |
|---|---|
| Codex の使い手 | Codex で動く依頼を処理する仕組み / Codex 用の依頼を受け取って処理するもの |
| consumer | 受け取って処理する仕組み |
| packet | 1 件の依頼ファイル / 受け渡しの 1 単位 |
| watcher | 監視する仕組み |
| dispatcher | 振り分ける仕組み |
| trigger | 起動の引き金 / 発火点 |

**判断基準**:
- 1 単語に短く訳せて、 かつ意味が自然に通る場合のみ訳語を採用 (例: failure → 失敗、 audit → 点検)
...
- path はホームディレクトリ起点 (`~/...`) もしくは絶対 path (`C:\...` / `/c/...`)、 相対 path は使わない
- 「commit済み」 「push済み」 だけ書いて path を書かない癖を直す
- 直しの後で再確認してほしい時も同様、 直したファイルの path を併記する

**理由**:
- ジュンさんの可処分時間 5-10 分前提で、 file を探す step を肩代わりすることで判断時間に充てられる
- 「どのファイル？」 と聞き返される 1 往復を抑止
- 報告 form 3 段 (やったこと / 結果 / これからどうするか) の 「結果」 欄に path 併記、 もしくは末尾に 「確認するファイル:」 セクション

**適用範囲**:
- chat の中で確認 / review / GO 判断を求める全場面
- board file 起稿後の通知
- 直しの後の再確認依頼
- 公開 docs 起稿後の見せる場面
